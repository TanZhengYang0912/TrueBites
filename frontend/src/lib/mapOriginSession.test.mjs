...
test("map origin keeps only validated coordinates in this browser tab", () => {
...
test("clearing a saved origin also clears mounted same-tab state", () => {
...
test("map origin rejects corrupt and out-of-range coordinates", () => {
  const values = installSessionStorage();
...
test("location survives guest login but clears on logout or account switch", () => {
  ...
  assert.equal(switchAccount({ user: { id: "user-b" } }), true);
