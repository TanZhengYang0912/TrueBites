 ...
 * per vendor for the COVER), this calls the Python AI service's
 * /api/extract-frames endpoint (backend/services/frameExtractor.py), which
 * downloads the actual video and returns several quality-filtered candidate
 * frames (blurry/dark/near-duplicate ones already discarded).
 *
 ...
 *   - The Python AI service must be running and reachable at AI_SERVICE_BASE
 *     (default http://localhost:8000) — this script does not start it.
 */
...
if (!CONFIRMED) {
  console.log(
    "This downloads each vendor's video (server-side, via the Python AI\n" +
    "service) and writes real storage/DB changes. Free, no API key — but\n" +
    "slow (tens of seconds per vendor) and a real write to the shared\n" +
    "vendor-images bucket + vendors table. Re-run with --yes once you're\n" +
    "ready (ideally try --limit=5 first)."
  );
  process.exit(0);
}
...
async function extractFrames(videoUrl) {
  ...
  if (payload.error && !payload.frames?.length) throw new Error(payload.error);
...
async function main() {
  try {
    const health = await fetch(`${AI_SERVICE_BASE}/health`, { signal: AbortSignal.timeout(5000) });
    if (!health.ok) throw new Error(`unexpected status ${health.status}`);
  } catch (err) {
    console.error(`❌  AI service not reachable at ${AI_SERVICE_BASE} (${err.message}).`);
    console.error("   Start it first — from backend/: python main.py (or uvicorn main:app --port 8000)");
    process.exit(1);
  }
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      const frames = await extractFrames(vendor.source_video_url);
      if (!frames.length) { console.log("⚠️  no usable frames extracted"); noFrames++; log.done.push(vendor.id); continue; }
      ...
      for (const frame of toUpload) {
        const { url } = await downloadAndStorePhoto(vendor.id, frame.url, "gallery");
        uploaded.push(url);
      }
